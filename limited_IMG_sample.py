    ...
    model, diffusion = CL_IMG_create_model_and_diffusion(
        **args_to_dict(
            ...
            [
                ...
                "condition_channels",
                # v2 骨干相关，必须与训练时一致，否则 state_dict 加载会失败
...
def create_argparser():
    defaults = dict(
        ...
        batch_size=1,
        # v2 的 ema_rate 默认是 0.999，训练总步数 37500，因此文件名形如
        # ema_npy_0.999_037500.pt；另一个档位 mode_npy_037500.pt 存的是原始权重。
        model_path="/home/lqg/code_8T/24/lt/CL_DIFF_v2/checkpoints/v2b_guss/ema_npy_0.999_037500.pt",
        output_dir="/home/lqg/code_8T/24/lt/CL_DIFF_v2/result/v2b_guss_037500",
        max_samples=0,
        slover_data="no",
        arch="v2b",
        image_size=768,
