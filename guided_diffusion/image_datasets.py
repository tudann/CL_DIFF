...
def validate_normalization_mode(mode):
    if mode not in ("slice", "volume", "percentile"):
        raise ValueError("normalization_mode must be 'slice', 'volume', or 'percentile'.")
    return mode
...
def volume_normalization_range(
    ...
    """Return the shared normalization range for a cropped 3-D volume."""
        ...
        for pair_idx, label_path in enumerate(self.label_paths):
            ...
            if self.normalization_mode in ("volume", "percentile"):
                self._normalization_ranges[label_path] = volume_normalization_range(
                    label_volume, self.crop_x, self.crop_y, self.normalization_mode,
                    self.percentile_low, self.percentile_high,
                )
                self._normalization_ranges[self.cond_paths[pair_idx]] = volume_normalization_range(
                    cond_volume, self.crop_x, self.crop_y, self.normalization_mode,
                    self.percentile_low, self.percentile_high,
                )
